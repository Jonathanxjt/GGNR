        ...
        return jsonify(
            {
                ...
                "data": {
                    "tickets": [
                        individual_ticket.json() for individual_ticket in ticket_list
                    ]
                },
            }
        )
...
def get_tickets_by_eid(EID):
    ...
    return (
        jsonify({"code": 404, "message": "There are no tickets for this event."}),
        404,
    )
...
def create_ticket():
    ...
    try:
        ...
        user_ticket = UserTicket(UID=uid, TicketID=ticket.TicketID)
