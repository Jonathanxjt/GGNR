...
# get the twilio account sid and auth token from the environment variable
...
def schedule_message_sending(contact, message_body, send_time_str):
    ...
    try:
        ...
        scheduler.add_job(
            send_sms, "date", run_date=send_time, args=[contact, message_body]
        )
...
def send_sms(contact, message_body):
    ...
    try:
        message = client.messages.create(
            body=message_body, from_="+19287560401", to=contact
        )
...
def processOrderLog(order):
    ...
    if "users" in order:
        ...
        for user in order["users"]["data"]:
            # iterate through the users and schedule the message
            contact = user["contact"]
            print(
                f"Scheduling message for User ID: {user['UID']}, Contact: {contact}, Email: {user['email']}, Username: {user['username']}"
            )
